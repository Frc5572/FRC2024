...
import static frc.lib.math.EqualsUtil.epsilonEquals;
...
public class SwerveSetpointGenerator {
    ...
    public SwerveSetpoint generateSetpoint(final ModuleLimits limits,
        ...
        if (EqualsUtil.GeomExtensions.epsilonEquals(GeomUtil.toTwist2d(desiredState),
            new Twist2d())) {
            need_to_steer = false;
        ...
        if (all_modules_should_flip
            && !EqualsUtil.GeomExtensions
                .epsilonEquals(GeomUtil.toTwist2d(prevSetpoint.chassisSpeeds()), new Twist2d())
            && !EqualsUtil.GeomExtensions.epsilonEquals(GeomUtil.toTwist2d(desiredState),
                new Twist2d())) {
    ...
    public static class SwerveSetpointGeneratorBuilder {
        private SwerveDriveKinematics kinematics;
        private Translation2d[] moduleLocations;

        SwerveSetpointGeneratorBuilder() {}

        /**
         * @return {@code this}.
         */
        public SwerveSetpointGenerator.SwerveSetpointGeneratorBuilder kinematics(
         ...
         * @return {@code this}.
         */
        public SwerveSetpointGenerator.SwerveSetpointGeneratorBuilder moduleLocations(
            final Translation2d[] moduleLocations) {
            this.moduleLocations = moduleLocations;
            return this;
        }

        public SwerveSetpointGenerator build() {
            return new SwerveSetpointGenerator(this.kinematics, this.moduleLocations);
        }

        @Override
        public String toString() {
            return "SwerveSetpointGenerator.SwerveSetpointGeneratorBuilder(kinematics="
                + this.kinematics + ", moduleLocations="
                + java.util.Arrays.deepToString(this.moduleLocations) + ")";
        }
    }

    public static SwerveSetpointGenerator.SwerveSetpointGeneratorBuilder builder() {
        return new SwerveSetpointGenerator.SwerveSetpointGeneratorBuilder();
    }
