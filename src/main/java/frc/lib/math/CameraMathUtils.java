...
public class CameraMathUtils {
    /**
     * Converts cartesian coordinates in camera frame to bearing/elevation that would be returned by
     * a pinhole camera model. X-forward, Y-left, Z-up.
     */
    ...
    /**
     * Returns the camera-frame vector given the bearing and elevation in a pinhole model.
     ...
     */
    public static Pair<Double, Double> XYToYawPitchWithHeightAndFOV(final double x, final double y,
    ...
    public static Pair<Double, Double> XYToYawPitch(final double x, final double y,
        final double centerX, final double centerY, final double horizontalFocalLength,
        final double verticalFocalLength) {
        return new Pair<>(Math.atan((x - centerX) / horizontalFocalLength),
            -Math.atan((y - centerY) / verticalFocalLength));
    }

    /** Converts yaw/pitch radians to X/Y pixels. +X right, +Y down. */
    public static Pair<Double, Double> yawPitchToXY(final double yaw, final double pitch,
        final double imageWidth, final double imageHeight, final double fovWidth,
        final double fovHeight) {
        final double centerX = (imageWidth / 2.0) - 0.5;
        final double centerY = (imageHeight / 2.0) - 0.5;
        final double horizontalFocalLength = imageWidth / (2.0 * Math.tan(fovWidth / 2.0));
        final double verticalFocalLength = imageHeight / (2.0 * Math.tan(fovHeight / 2.0));
        return new Pair<>((Math.tan(yaw) * horizontalFocalLength) + centerX,
