...
public class ApriltagVisionIOSim extends AprilTagVisionIOReal {
    ...
    public ApriltagVisionIOSim(List<PhotonCameraProperties> cameraProperties,
        ...
        for (int i = 0; i < cameraProperties.size(); i++) {
            final PhotonCameraSim cameraSim = new PhotonCameraSim(super.cameras[i],
                cameraProperties.get(i).getSimulationProperties());
            cameraSim.enableRawStream(true);
            cameraSim.enableProcessedStream(true);
            cameraSim.enableDrawWireframe(true);
            visionSystemSim.addCamera(camerasSim[i] = cameraSim,
