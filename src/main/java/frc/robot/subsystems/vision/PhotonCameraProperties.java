...
public class PhotonCameraProperties {
    ...
    /**
     * Photon Camera Properties
     *
     * @param name Camera Name. Must match the name in Photon Vision
     * @param ipAddress Camera IP for using the rest API
     * @param frameRate
     * @param averageLatencyMS
     * @param latencyStandardDeviationMS
     * @param cameraFOVDegreesDiag
     * @param calibrationAverageErrorPixel
     * @param calibrationErrorStandardDeviation
     * @param captureWidthPixels
     * @param captureHeightPixels
     * @param mountPositionOnRobotMeters
     * @param mountHeightMeters
     * @param cameraFacing
     * @param cameraPitchDegrees
     * @param rotateImageDegrees
     */
    public PhotonCameraProperties(String name, String ipAddress, double frameRate,
        double averageLatencyMS, double latencyStandardDeviationMS, double cameraFOVDegreesDiag,
        double calibrationAverageErrorPixel, double calibrationErrorStandardDeviation,
        int captureWidthPixels, int captureHeightPixels, Translation2d mountPositionOnRobotMeters,
        double mountHeightMeters, Rotation2d cameraFacing, double cameraPitchDegrees,
        double rotateImageDegrees) {
        this(name, ipAddress, frameRate, averageLatencyMS, latencyStandardDeviationMS,
            Rotation2d.fromDegrees(cameraFOVDegreesDiag), calibrationAverageErrorPixel,
            calibrationErrorStandardDeviation, captureWidthPixels, captureHeightPixels,
            new Transform3d(mountPositionOnRobotMeters.getX(), mountPositionOnRobotMeters.getY(),
                mountHeightMeters,
                new Rotation3d(0, -Math.toRadians(cameraPitchDegrees), cameraFacing.getRadians()))
                    .plus(new Transform3d(new Translation3d(),
                        new Rotation3d(Math.toRadians(rotateImageDegrees), 0, 0))));
    }

    public PhotonCameraProperties(String name, String ipAddress, double frameRate,
        double averageLatencyMS, double latencyStandardDeviationMS, Rotation2d cameraFOVDiag,
        double calibrationAverageErrorPixel, double calibrationErrorStandardDeviation,
        int captureWidthPixels, int captureHeightPixels, Transform3d robotToCamera) {
        this.name = name;
    ...
    public SimCameraProperties getSimulationProperties() {
        ...
        cameraProperties.setCalibError(calibrationAverageErrorPixel,
    ...
    // } catch (IOException e) {
    // DriverStation.reportError("cannot find camera properties config file: " + configName
    // + " from deploy directory", true);
    // return new ArrayList<>();
    // }
    // return loadCamerasPropertiesFromConfig(configFile);
    // }

    // public static List<PhotonCameraProperties> loadCamerasPropertiesFromConfig(
    // MapleConfigFile configFile) {
    ...
    // cameraProperties.add(loadSingleCameraPropertyFromBlock(cameraBlock, cameraFPS,
    // averageLatencyMS, latencyStandardDeviationMS));
    // }
    ...
    // MapleConfigFile.ConfigBlock cameraBlock, double cameraFPS, double averageLatencyMS,
    ...
    // new Rotation3d(0, -Math.toRadians(cameraBlock.getDoubleConfig("cameraPitchDegrees")),
    ...
    // default -> 0;
    // };
    // final Transform3d cameraInstallationToCapturedImage = new Transform3d(new Translation3d(),
    // new Rotation3d(Math.toRadians(cameraRollDeg), 0, 0));
    // return new PhotonCameraProperties(cameraBlock.getStringConfig("name"), cameraFPS,
    // averageLatencyMS, latencyStandardDeviationMS,
    ...
    // public static MapleConfigFile createConfigFileForCameras(String name, int camerasAmount,
    // double cameraFPS, double averageLatencyMS, double latencyStandardDeviationMS,
    // int cameraWidthPixels, int cameraHeightPixels, double defaultCameraFOVDegrees) {
    ...
    // createConfigBlockForCamera(configFile.getBlock("Camera" + i), cameraWidthPixels,
    // cameraHeightPixels, defaultCameraFOVDegrees);
    // }

    // return configFile;
    // }

    // private static void createConfigBlockForCamera(MapleConfigFile.ConfigBlock cameraBlock,
    // int cameraWidthPixels, int cameraHeightPixels, double cameraFOVDegrees) {
    ...
    // calibrationAverageErrorPixel, calibrationErrorStandardDeviation,
    // cameraFOVDiag.toString(), captureWidthPixels, captureHeightPixels,
